...
class ShotsController extends Controller
{
    ...
    {
        $SubCategorias = SubCategoria::porCategoria('shots');

        // $subCategorias = SubCategoria::all();
        return view('back.shots.create', [
     ...
     */
    public function edit(Producto $shot)
    {

        $SubCategorias = SubCategoria::porCategoria('shots');

        return view('back.shots.edit', [
            'shot' => $shot,
            'SubCategorias' => $SubCategorias,
        ]);
    }
    ...
    public function update(Request $request, Producto $shot)
    {
        // Validación
        $request->validate([
            'nombre' => 'required|min:5|max:30',
        ...
        $shot->update($request->only(['nombre', 'precio', 'descripcion', 'id_sub_categoria']));

        // Reemplazar imagen si subieron una nueva
        if ($request->hasFile('imagen')) {
            // Eliminar imagen anterior si existe
            if ($shot->imagen && Storage::disk('public')->exists($shot->imagen)) {
                Storage::disk('public')->delete($shot->imagen);
            }

            // Guardar la nueva imagen y asignar ruta al modelo
            $shot->imagen = $request->file('imagen')->store('photos', 'public');
        }
        ...
        while (Producto::where('slug', $slug)->where('id', '!=', $shot->id)->exists()) {
            $slug = $originalSlug . '-' . $contador;
            $contador++;
        }
     ...
     */
    public function destroy(Producto $shot)
    {
