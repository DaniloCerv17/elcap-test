...
class TemporadaController extends Controller
{
     public function index()
    {
        ...
        return view('back.temporada.index', [
    ...
    {
        $SubCategorias = SubCategoria::porCategoria('temporada');

        // $subCategorias = SubCategoria::all();
        return view('back.temporada.create', [
    ...
    public function store(Request $request)
    {
        ...
        return redirect('back/temporada');
    }
     ...
     */
    public function edit(Producto $temporada)
    {

        $SubCategorias = SubCategoria::porCategoria('temporada');

        return view('back.temporada.edit', [
            'temporada' => $temporada,
            'SubCategorias' => $SubCategorias,
        ]);
    }
    ...
    public function update(Request $request, Producto $temporada)
    {
        // Validación
        $request->validate([
            'nombre' => 'required|min:5|max:30',
        ...
        $temporada->update($request->only(['nombre', 'precio', 'descripcion', 'id_sub_categoria']));

        // Reemplazar imagen si subieron una nueva
        if ($request->hasFile('imagen')) {
            // Eliminar imagen anterior si existe
            if ($temporada->imagen && Storage::disk('public')->exists($temporada->imagen)) {
                Storage::disk('public')->delete($temporada->imagen);
            }

            // Guardar la nueva imagen y asignar ruta al modelo
            $temporada->imagen = $request->file('imagen')->store('photos', 'public');
        }
        ...
        while (Producto::where('slug', $slug)->where('id', '!=', $temporada->id)->exists()) {
            $slug = $originalSlug . '-' . $contador;
            $contador++;
        }
     ...
     */
    public function destroy(Producto $temporada)
    {
        ...
        // Eliminar el shot de la base de datos
        $temporada->delete();
